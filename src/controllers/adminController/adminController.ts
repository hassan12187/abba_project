import mongoose, { startSession, type ObjectId } from "mongoose";
...
export const approveStudentApplication=async(req:Request,res:Response)=>{
...
interface StudentFilter {
    status?: string | { $or: { status: string }[] } | any;
    $or?: any[];
    $and?: any[];
    student_cellphone?: number;
    student_roll_no?: number;
    student_name?: { $regex: string; $options: string };
}
export const getStudentApplications=async(req:Request,res:Response)=>{
   try {
        ...
        const defaultStatusFilter=[{status:"pending"},{status:"rejected"}];
        if(status && status !== "approved"){
            filter.status=status;
        }else{
            filter=  {   $or:defaultStatusFilter
                        };
        }
        if(query){
            let isNum=!isNaN(Number(query));
            if(isNum){
                filter={
                    $and:[
                        {
                            $or:defaultStatusFilter
                        },
                        {
...
interface GetAllStudentsFilterKeyStatus{
    status?:string |{$or?:{status:string}[]} |null;
    $or?:any[];
    student_cellphone:string|null;
    student_name:{$regex:string,$options:string};
    room_id:string | null;
};
export const getAllStudents=async(req:Request,res:Response)=>{
    try {
        ...
        const query = (req.query.query as string)||"";
        const status =(req.query.status as string)||"" ;
        const room_assign = (req.query.room_assign as string)||"";
        // console.log(status);
       ...
       let filterKey:Partial<GetAllStudentsFilterKeyStatus> = {};
    ...
    if (query) {
      const isNumber = !isNaN(Number(query)); // true if numeric
      if (isNumber) {
...
export const assignRoom=async(req:Request,res:Response)=>{
    ...
    try {
            ...
            if(!room || (room.capacity - room.occupants.length)<=0){
                console.log("condition true");
                await session.abortTransaction();
                await session.endSession();
                return res.sendStatus(400);
            };
            room.occupants.push(new mongoose.Types.ObjectId(id));
