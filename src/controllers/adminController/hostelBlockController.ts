...
export const handleGetAllBlocks=async(req:Request,res:Response)=>{
    try {
...
export const handleGetBlocks=async(req:Request,res:Response)=>{
    try {
        ...
        const cachedData=await redis.get(cachedKey);
        if(cachedData)return res.status(200).json({data:JSON.parse(cachedData)});
        let filterKey:Partial<{
            block_no:{$regex:string,$options:string},
            status:string,
            $and:[{block_no:{$regex:string,$options:string},status:string}]
        }>={};
        if(block){
...
export const handleAddBlock=async(req:Request,res:Response)=>{
    try {
...
export const handleGetBlock=async(req:Request,res:Response)=>{
    try {
        const {id} = req.params;
...
export const handleEditBlock=async(req:Request,res:Response)=>{
    try {
        const {id}=req.params;
